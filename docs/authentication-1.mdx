...
<Warning>
  ...
  *While you can assign`create`, `update`, and `delete` scopes and your token will include them, the corresponding write endpoints are **not yet available**. At this time, the Public API only exposes **read** operations—those additional scopes are in place for seamless adoption once write functionality is release*
</Warning>
