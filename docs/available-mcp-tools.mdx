...
| Tool | Tier | Scope | Description |
| --- | --- | --- | --- |
| `trips_search` | Read | `trip:read` | Search trips by status, load number, trip number, and/or pickup/delivery date range (`pickupFrom`/`pickupTo`, `deliveryFrom`/`deliveryTo`, ISO-8601). At least one filter is required; a date range alone is sufficient. |
...
| Tool | Tier | Scope | Description |
| --- | --- | --- | --- |
| `customers_search` | Read | `customer:read` | Search customers by status and/or record-creation date range (`createdFrom`/`createdTo`, ISO-8601). Results are ordered oldest-first by record-creation date. There is no name filter — page and match client-side. |
...
| `trucks_events_search` | Read | `truck:read` | Fetch truck events (maintenance, schedule, availability) for one or more trucks over a date range. Pass `truckIds[]` (Alvys truck ids, not unit numbers) and a `startDate`; `endDate` is optional. Returns a flat event list — an empty list means no events in the range. |
...
| Tool | Tier | Scope | Description |
| --- | --- | --- | --- |
| `invoices_search` | Read | `invoice:read` | Search invoices by customer, status, load number, or order number, optionally narrowed by invoiced-date (`invoicedFrom`/`invoicedTo`) and/or paid-date (`paidFrom`/`paidTo`) ranges. At least one of `customerId`, `status`, `loadNumber`, or `orderNumber` is required — date ranges narrow results but do not count as the sole filter. `invoicedFrom`/`invoicedTo` filters on invoice-record creation date (includes `Draft`), so pair the range with a status filter for billed-amount questions. |
