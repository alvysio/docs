...
description: "Import live Alvys loads, trips, customers, drivers, carriers, and invoices into Google Sheets using the Public API template, with no coding required."
---
...
<Warning>
    Important: Make sure your application credentials has all required **read permissions.**
...
* ⚠️For large data imports, run each entity (Loads, Trips, Customers, etc.) manually and one at a time — wait for each to finish before starting another.
