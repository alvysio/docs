...
description: "Approve carrier payable items, generate carrier settlement statements, email statements to carriers, and sync carrier pay to QuickBooks or NetSuite."
---
...
Carrier Settlements integrates with QuickBooks Online, QuickBooks Desktop, Sage Intacct, and NetSuite through your existing accounting integration settings.
...
### Reference number on synced bills and payments

When a carrier settlement statement syncs to your accounting system, the reference number on the resulting bill or payment is prefixed with `s-` (for example, `s-1234` or `s-TRIP-777`). The prefix appears in:

- **QuickBooks Online** — the `Doc Number` field on the bill or payment.
- **QuickBooks Desktop** — the `Ref Number` field on the bill or payment.
- **Sage Intacct** — the `Bill Number` field.
- **NetSuite** — the `Transaction ID` on the bill or payment.

The prefix makes carrier settlement transactions easy to identify and filter alongside other bills and payments in your accounting system. Only the reference number changes — the memo (`Statement {N}`), trip number, carrier invoice number, and PO fields are not affected.

The prefix is recorded on the statement at the moment it is generated, so:

- Statements generated before this change stay as-is (no prefix).
- After a statement is generated, changing the setting does not update statements that already exist.

<Note>
  If your workflow requires the reference number to stay as a bare number (with no `s-` prefix), contact Alvys support and ask to opt out of the carrier statement reference prefix for your tenant. New statements generated after opt-out will sync without the prefix; existing statements are unchanged.
</Note>
...
**Q: Why do carrier settlement bills and payments show up in my accounting system with an `s-` prefix?**

**A:** By default, Alvys prefixes the reference number on synced carrier settlement bills and payments with `s-` so they're easy to distinguish from other transactions (for example, `s-1234` appears in the QuickBooks Online `Doc Number`, QuickBooks Desktop `Ref Number`, Sage Intacct `Bill Number`, or NetSuite `Transaction ID`). The memo and other fields are unchanged. If you need bare reference numbers, contact Alvys support to opt out for your tenant; the opt-out applies to statements generated after the change and does not affect statements that already synced.
