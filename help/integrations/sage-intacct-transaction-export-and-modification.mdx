...
#### Changing the Subsidiary on a Posted Transaction

If you change the **Invoice As**, **Tender As**, or driver subsidiary on a load after its transaction has already been exported — and the new subsidiary maps to a **different Sage Intacct entity** than the original — Alvys reverses the prior invoice or bill under the **original entity** before posting the corrected transaction under the **new entity**. The original posting is not left orphaned in the previous entity's ledger.

<Check>
  💡 This applies to any subsidiary change that crosses Sage entities, including moves between a top-level entity and a child entity. If the previous posting is in **Paid**, **Partially Paid**, **Closed**, or **Selected** state, the same restrictions in the list above apply — clear the blocking state in Sage first, then re-sync the transaction from **Accounting → Error Transactions**.
</Check>
