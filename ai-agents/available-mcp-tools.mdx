...
description: "Full catalog of tools exposed by the Alvys MCP server for AI agents, grouped by domain with required OAuth scopes and permissions."
---

This page lists every tool the Alvys [MCP server](/ai-agents/mcp) exposes to AI agents. Each tool maps 1:1 to an Alvys Public API capability and requires a specific permission (scope). Your token must carry that scope for the call to succeed.
...
Scopes use the `{resource}:{action}` convention and match the [Public API scope catalog](/docs/authentication#available-scopes). Assign them in **Admin → API Access**.
...
<CardGroup cols={2}>
  <Card title="MCP overview" icon="plug" href="/ai-agents/mcp">
    Connection setup and authentication options.
  </Card>
